...
def save_model(PATH):
    ...
    model.load_state_dict(checkpoint['state_dict'])
    model.eval()
    torch.save(model,'resnet20_pretrained')
...
def set_layer(model,name,layer):
    attr,name = name.rsplit('.',1)
    model = get_layer(model,attr)
...
def decomposition():
    ...
    for name,layer in model.named_modules():
        #for i,(name,layer) in enumerate(layers.named_modules()):
            if isinstance(layer, torch.nn.modules.conv.Conv2d):
                conv_layer = layer
                decomposed = tucker_decomposition_conv_layer(conv_layer)
                #setattr(layers,name,nn.Sequential(*decomposed))
